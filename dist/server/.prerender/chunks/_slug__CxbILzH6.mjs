...
import { D as DirectusAuthorsSchema, a as DirectusAuthorsSlugSchema, b as DirectusAuthorSchema, c as DirectusArticlesCollectionResponseSchema } from './index_Bh7jurQE.mjs';
import { $ as $$PostCard } from './PostCard_Cu16gFvF.mjs';
import 'clsx';

const $$PostAuthors = createComponent(async ($$result, $$props, $$slots) => {
  const catUrl = `${"https://core-cms.core-hub-plex.cloud/items"}/authors`;
...
async function getStaticPaths() {
  const newRes = await fetch(`${"https://core-cms.core-hub-plex.cloud/items"}/authors`);
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const autorUrl = `${"https://core-cms.core-hub-plex.cloud/items"}/authors?filter[slug][_eq]=${slug}`;
  ...
  const postsUrl = `${"https://core-cms.core-hub-plex.cloud/items"}/articles?filter[author][_eq]=${autor.id}&fields=*,article_category.*,article_tags.*,author.*`;
  const postsRes = await fetch(postsUrl);
  const postsJson = await postsRes.json();
  const posts = DirectusArticlesCollectionResponseSchema.parse(postsJson);
  const { data: postsAuthor } = posts;
  return renderTemplate`${renderComponent($$result, "Layout", $$CoreLayout, { "title": `Posts por el autor: ${autor.name}`, "subtitle": `Posts por el autor: ${autor.name}`, "bgImage": `${"https://core-cms.core-hub-plex.cloud/assets"}/${postsAuthor[0]?.cover_image || "default-bg.jpg"}` }, { "default": async ($$result2) => renderTemplate` ${renderComponent($$result2, "AuthorSummary", $$AuthorSummary, { "autor": autor })} ${maybeRenderHead()}<h2 class="mx-10 md:mx-auto my-10 text-3xl md:text-5xl font-black uppercase text-white flex items-center gap-4"> <span class="w-8 h-8 bg-neon-pink inline-block neo-shadow-cyan"></span>
Posts del autor:
