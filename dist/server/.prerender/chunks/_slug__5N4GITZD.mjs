...
async function getStaticPaths() {
  const res = await fetch(`${"https://core-cms.core-hub-plex.cloud/items"}/articles_tags`);
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const tagUrl = `${"https://core-cms.core-hub-plex.cloud/items"}/articles_tags?filter[slug][_eq]=${slug}`;
  const tagRes = await fetch(tagUrl);
  const tagJson = await tagRes.json();
  const tag = DirectusCategorySchema.parse(tagJson.data[0]);
  const postsUrl = `${"https://core-cms.core-hub-plex.cloud/items"}/articles?filter[article_tags][_eq]=${tag.id}&fields=*,article_category.*,article_tags.*,author.*`;
