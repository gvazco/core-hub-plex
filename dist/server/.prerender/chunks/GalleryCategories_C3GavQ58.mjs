...
import 'clsx';
import { k as DirectusGalleriesCategoryResponseSchema } from './index_Bh7jurQE.mjs';

const $$GalleryCategories = createComponent(async ($$result, $$props, $$slots) => {
  const newCatUrl = `${"https://core-cms.core-hub-plex.cloud/items"}/galleries_categories`;
  ...
  return renderTemplate`${maybeRenderHead()}<div class="mt-5 my-3 slider overflow-y-hidden" data-aos="zoom-in-up" data-aos-duration="2000"> <div class="list"> ${catData.filter((cat) => cat.status === "published").map((cat) => renderTemplate`<div class="item"> <a${addAttribute(`/galerias/categoria/${cat.slug}`, "href")} class="inline-block py-1 px-5 text-sm text-white bg-dark neo-border-pink p-5 neo-shadow-pink neo-hover-pink transition-neo"> ${cat.name} </a> </div>`)} ${catData.filter((cat) => cat.status === "published").map((cat) => renderTemplate`<div class="item"> <a${addAttribute(`/galerias/categoria/${cat.slug}`, "href")} class="inline-block py-1 px-5 text-sm text-white bg-dark neo-border-pink p-5 neo-shadow-pink neo-hover-pink transition-neo"> ${cat.name} </a> </div>`)} </div> </div>`;
