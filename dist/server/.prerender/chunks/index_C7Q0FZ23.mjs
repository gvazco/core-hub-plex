import { c as createComponent, r as renderScript, $ as $$CoreLayout } from './CoreLayout_a_vsr70c.mjs';
import 'piccolore';
import { m as maybeRenderHead, e as addAttribute, b as renderTemplate, r as renderComponent } from './prerender_Hd2AncPp.mjs';
import 'clsx';
import { l as DirectusHotNewsResponseSchema, m as DirectusPageIndexResponseSchema } from './index_Bh7jurQE.mjs';
...
const $$TickerHotNews = createComponent(async ($$result, $$props, $$slots) => {
  const newURL = `${"https://core-cms.core-hub-plex.cloud/items"}/hot_news`;
  const newResponse = await fetch(newURL);
  const newJson = await newResponse.json();
  const newData = DirectusHotNewsResponseSchema.parse(newJson);
  const { data: dataItems } = newData;
  return renderTemplate`<!----------------------- Ticker de Noticias Rápidas ----------------------->${maybeRenderHead()}<div class="mt-0 bg-neon-yellow text-dark py-2 border-b-4 border-neon-yellow overflow-hidden font-mono font-bold text-lg flex items-center slider"${addAttribute(`--width: 600px; --height: 50px; --quantity: ${dataItems.length}; `, "style")}> <div class="list"> ${dataItems.map(
    (item) => item.status === "published" && renderTemplate`<div class="item"${addAttribute(`--position: ${item.id}`, "style")}> <a class="flex flex-row"${addAttribute(item.article_url, "href")} target="_blank"> <span class="material-symbols-outlined mr-2">${item.icon}</span> <p> ${item.title} <span>${item.subtitle}</span> </p> </a> </div>`
  )} </div> </div>`;
...
const $$AdvertisingSpace = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${maybeRenderHead()}<div data-aos="zoom-in-up" data-aos-duration="2000" id="carousel" class="carousel-container"> <div class="carousel-slides"> <div class="carousel-slide active"> <div class="p-8 border-4 border-neon-yellow border-dashed flex flex-col items-center justify-center text-center flex-grow"> <h3 class="text-2xl font-bold text-neon-yellow uppercase mb-2">
ANÚNCIATE AQUÍ
...
const $$CoreFeaturedContent = createComponent(async ($$result, $$props, $$slots) => {
  const newUrl = `${"https://core-cms.core-hub-plex.cloud/items"}/page_index?fields=*.*.*`;
  const newResponse = await fetch(newUrl);
  const newJson = await newResponse.json();
