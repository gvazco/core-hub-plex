...
async function getStaticPaths() {
  const res = await fetch(`${undefined                         }/galleries_tags`);
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const tagUrl = `${undefined                         }/galleries_tags?filter[slug][_eq]=${slug}`;
  const tagRes = await fetch(tagUrl);
  const tagJson = await tagRes.json();
  const tag = DirectusCategorySchema.parse(tagJson.data[0]);
  const postsUrl = `${undefined                         }/galleries?filter[gallery_tag][_eq]=${tag.id}&fields=*,gallery_category.*,gallery_tag.*,author.*,gallery.directus_files_id.*`;
